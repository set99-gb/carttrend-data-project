...
-- + Identification des 3 meilleures catégories combinées (vraies stars)
-- ============================================================================
...
classement AS (
  SELECT
    ...
    RANK() OVER (ORDER BY v.quantite_totale + v.chiffre_affaires DESC) AS rang_global
  FROM ventes_cat v
)

-- Étape 4 : Segmentation enrichie
SELECT
  c.categorie,
  c.quantite_totale,
  c.chiffre_affaires,
  ...
  s.mediane_ca,
  CASE
    WHEN c.rang_global <= 3 THEN 'Vraie Star'
